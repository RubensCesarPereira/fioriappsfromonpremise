sap.ui.define([
	...
	return BaseObject.extend("cl.conchaytoro.zpp_elaboracion_mezcla.model.GroupSortState", {
		 ...
		 * @alias cl.conchaytoro.zpp_elaboracion_mezcla.model.GroupSortState
		 */
		...
		/**
		 * Groups by Gsmng, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "Gsmng") {
				...
				this._oViewModel.setProperty("/sortBy", "Gsmng");

				aSorters.push(
					new Sorter("Gsmng", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "Psttr");
			}
