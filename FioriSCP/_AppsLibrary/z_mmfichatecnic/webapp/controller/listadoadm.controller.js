sap.ui.define([
	...
	return BaseController.extend("z_mmfichatecnic.controller.listadoadm", {
		onInit : function() {	
			var oRouter = sap.ui.core.UIComponent.getRouterFor(this);
			oRouter.getRoute("listadoadm").attachMatched(this._onRouteFound, this);
		},
		...
		onPress:function(oEvent){
			this.getModel('CONF').setProperty('/MODE','ADM');
			var oContext = oEvent.getSource().getBindingContext('ADMLIST');
			var id = oContext.getProperty("MATNR");
			this.getRouter().navTo("ficha",{id:id});
		},
		getListA: function(){
			var date_range = this.byId('range-adm-date');
			...
			var state = this.byId('state-adm-select').getSelectedKey();
			var cod = this.byId('adm-cod').getValue();
			console.log("Estado: "+state+" codigo: "+cod);
			var pag = this.byId('pag');
			pag.setBusy(true);
			var oModel = new sap.ui.model.odata.ODataModel(serviceUrl,true);
			oModel.read("/ListadoAdmSet?$filter=(ERSDA ge '"+dInit+"' and ERSDA le '"+dEnd+"' and ESTADO eq '"+state+"' and MATNR eq '"+cod+"')",
					null, ["" ], true,
				...
				oView.setModel(oJsonModel,"ADMLIST");
			}, function(err) {
				pag.setBusy(false);
			...
			var oDialog = new Dialog({
				...
				content: [
					new sap.m.FlexBox({
						...
						alignItems: 'Center',
						items: [new sap.m.Select('select-vina-pdfDos',{
							items:items
					...
					new sap.m.HBox({
						items : [
							new Button({
								...
								type: 'Ghost',
								press : function(){
									var sKey = sap.ui.getCore().byId('select-vina-pdfDos').getSelectedKey();
