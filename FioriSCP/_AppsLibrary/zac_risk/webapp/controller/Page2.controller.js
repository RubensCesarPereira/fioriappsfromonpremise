sap.ui.define([
		...
		"use strict";
		return BaseController.extend("zac.grc.risk.cust.controller.Page2", {
			handleRouteMatched: function (oEvent) {
				...
				var gerencia = sap.ui.getCore().myGlobalVarg;
				var sgerencia = sap.ui.getCore().myGlobalVarsg;
				//PERIODO
				var resultado = sap.ui.getCore().myGlobalVarp;
				var sociedad = sap.ui.getCore().myGlobalVars;

				var txtSubGerencia = "Riesgos por gerencia: " + gerencia + " - Periodo: " + resultado;
				var oView = this.getView();
				this.getView().byId("txtSubGerencia_id").setText(txtSubGerencia);
				...
				if (resultado) {
					GerenciaFilters.push(new sap.ui.model.Filter("Gerencia", sap.ui.model.FilterOperator.EQ, gerencia));
					/*if (sgerencia) {
						GerenciaFilters.push(new sap.ui.model.Filter("SGerencia", sap.ui.model.FilterOperator.EQ, sgerencia));	
					}*/
					GerenciaFilters.push(new sap.ui.model.Filter("Periodo", sap.ui.model.FilterOperator.EQ, oRiesgoPeriodo));
					GerenciaFilters.push(new sap.ui.model.Filter("Sociedad", sap.ui.model.FilterOperator.EQ, sociedad));
				}
				...
				this.byId("tblSGerencia_id").getBinding("items").filter(oFilter, "Application");
			},
			...
			doNavigate: function (sRouteName, oBindingContext, fnPromiseResolve, sViaRelation) {
				...
				if (oBindingContext) {
					...
					var periodo = sap.ui.getCore().myGlobalVarp;
			...
			onObjectNumberPress: function (oEvent) {
				...
				var url2 = "/sap/opu/odata/sap/ZAC_RISK_SRV/SGerenciaSet?$filter=Periodo eq ('" + oRiesgoPeriodo + "')" + " and Gerencia eq '" + gerencia + "'" + " and Sociedad eq '" + sociedad + "'" + "&$format=xlsx";
				window.open(url2);
			},
			...
			_onRowPress1: function (oEvent) {
				...
				return new Promise(function (fnResolve) {
					...
					var propertySociedad = oBindingContext.getProperty("Sociedad");
					var gerencia = propertyGerencia;
					var subgerencia = propertySGerencia;
					var sociedad = propertySociedad;
					var periodo = sap.ui.getCore().myGlobalVarp;
			...
			*/
			...
			action: function (oEvent) {
				...
				GerenciaFiltros.push(new sap.ui.model.Filter("SGerencia", sap.ui.model.FilterOperator.Contains, searchTextValor));
				...
				this.byId("tblSGerencia_id").getBinding("items").filter(oFiltros, "Application");
				this.byId("tblSGerencia_id").getBinding("items").applyFilter();
				}else{
				...
				this._oGlobalFilter = null;
				var GerenciaFilters = [];
				var oRiesgoPeriodo = sap.ui.getCore().myGlobalVarp;
				if (sQuery) {
				GerenciaFilters.push(new sap.ui.model.Filter("Periodo", sap.ui.model.FilterOperator.EQ, oRiesgoPeriodo));
				}
				this._oGlobalFilter = GerenciaFilters;
		...
		handleConfirm: function(oEvent) {
			...
			var oView = this.getView();
			var oTable = oView.byId("tblSGerencia_id");
