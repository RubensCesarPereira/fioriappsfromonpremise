						...
						success : function (aEntitySet) {
							...
							return this.waitFor({
								...
								check: function (oList) {
									...
									var sObjectID = oList.getBindingContext().getProperty("Purchaseorder");

									var iLength = aEntitySet.filter(function (oLineItem) {
										return oLineItem.Purchaseorder === sObjectID;
									}).length;
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							return this.waitFor({
								id : "lineItemsHeader",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name: "text", value: "<PedidoToPosicionesPlural> (" + iNumberOfItems + ")"}),
								success : function () {
