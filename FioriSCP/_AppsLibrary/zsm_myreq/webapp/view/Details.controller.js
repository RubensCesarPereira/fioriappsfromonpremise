...
sap.ui.define([
	...
	"sap/m/MessageBox",
	"zsm/itsm/createrequirement/cust/util/Util"
], function(Controller, MessageBox, Util) {
	"use strict";

	return Controller.extend("zsm.itsm.createrequirement.cust.view.Details", {
		...
		_onRoutePatternMatched: function(oEvent) {
			 ...
			 * @callback zsm.itsm.createrequirement.cust.view.Details~extHookOnIncidentTypeSelection
		...
		onCancel: function() {
			if (Util.hasCancelListener()) {
				this.eventBus.publish("zsm.itsm.myreq.cust", "afterCancel");
			} else {
				Util.navToLaunchpad();
			}
		},
		...
		onCategoryValueHelp: function(oEvent) {
			this._oPopover = sap.ui.xmlfragment("categoryPopover", "zsm.itsm.createrequirement.cust.view.fragments.CategoryPopover", this);
		...
		onComponentValueHelp: function(oEvent) {
			this._oPopover = sap.ui.xmlfragment("componentPopover", "zsm.itsm.createrequirement.cust.view.fragments.ComponentPopover", this);
			this._oPopover.setModel(this.getView().getModel());
			this._oPopover.setModel(this.getView().getModel("i18n"), "i18n");
			jQuery.sap.syncStyleClass("sapUiSizeCompact", this.getView(), this._oPopover);
		...
		// 	if (this.eventBus.getInterface()._mChannels["zsm.itsm.createrequirement.cust"]) {
		// 		if (this.eventBus.getInterface()._mChannels["zsm.itsm.createrequirement.cust"].hasListeners("afterCreate")) {
		// 			bReturn = true;
		// 		}
		// 	}
		// 	return bReturn;
		...
		// 	if (this.eventBus.getInterface()._mChannels["zsm.itsm.createrequirement.cust"]) {
		// 		if (this.eventBus.getInterface()._mChannels["zsm.itsm.createrequirement.cust"].hasListeners("afterCancel")) {
		// 			bReturn = true;
		// 		}
		// 	}
		// 	return bReturn;
		...
		// 		var semanticObject = "display";
		// 		var action = "ZMyRequeriments";
