...
sap.ui.define([
	...
	return BaseController.extend("zsm.itsm.myreq.cust.view.S3", {
		...
		onPost: function(oEvent) {
			...
			//	var path = oCtx.sPath + "?$expand=StatusSet";
				...
				that.oView.notesfltr.setCount(that.oTextSet.length);
					...
					urlParameters:{"$expand" : "StatusSet" },
		...
		readAttachments: function(oView) {
			...
			var fnSuccess = function(oResponse) {
				...
				$.each(that.oAttachmentSet, function(index, value) {
                    ...
                    var url = value.__metadata.media_src.split("ZAI_CRM_GW_MYREQUIREMENTS_SRV");
                    var urlbase = window.location.href.split("cp.portal");
                    that.oAttachmentSet[index].url = urlbase[0] + "zsmitsmmyreqcust/sap/opu/odata/sap/ZAI_CRM_GW_MYREQUIREMENTS_SRV" + url[1];
		...
		createPopover: function(oModel) {
			if (!this._oQuickView) {
				this._oQuickView = sap.ui.xmlfragment("zsm.itsm.myreq.cust.view.fragments.QuickView", this);
		...
		openDialog: function(sType) {
			...
			if (!this[sType]) {
				this[sType] = sap.ui.xmlfragment(
					"zsm.itsm.myreq.cust.view.fragments." + sType + "Dialog",
